...
static simplejs_ast_node_type_t simplejs_get_node_type_for_unary_operator(simplejs_utf8_string_t *string, int string_cut, bool assign_first)
{
    ...
    for (size_t i = 0; i < sizeof(unary_operators_type_node) / sizeof(*unary_operators_type_node); i++)
    {
        ...
        if (strlen(entry->string) == len &&
            !strncmp(entry->string, string->buffer, len))
        {
...
static bool simplejs_get_scoped_reference_callback(simplejs_ast_scope_context_t *scope_context, void *context, void *out, bool is_out_of_function)
{
    ...
    simplejs_parser_ctx_t *parser_ctx, simplejs_ast_node_t *current_ast_stack, simplejs_ast_node_t **out, simplejs_token_t *token, bool is_property)
{
    ...
    bool is_class_scope = !local_scoped.is_argument && (current_function_context->ast_node->flags & SIMPLEJS_AST_NODE_FLAG_IS_CLASS_AST_NODE);
    ...
    else if (is_local && !is_class_scope)
    {
    ...
    {
        simplejs_ast_node_type_t a_type = SIMPLEJS_AST_NODE_TYPE_GLOBAL_REFERENCE;
        if (is_class_scope)
            a_type = SIMPLEJS_AST_NODE_TYPE_THIS_REFERENCE;

        SIMPLEJS_REQUIRE_SUCCESS(simplejs_alloc_ast_node(a_type, &a_ast), result, status);
