...
simplejs_status_t SIMPLEJS_PLATFORM_API simplejs_create_map_buffer_file(simplejs_map_buffer_t **out, char *file_path, uint32_t flags)
{
    ...
    char mode_str[8] = {0};
    if (flags & SIMPLEJS_MAP_BUFFER_FILE_FLAG_READ)
        mode_str[strlen(mode_str)] = 'r';
    if (flags & SIMPLEJS_MAP_BUFFER_FILE_FLAG_WRITE)
        mode_str[strlen(mode_str)] = '+';

    file = simplejs_file_open(file_path, mode_str);
    if (!file)
    {
